...
function warnOnce(paletteName, error) {
  ...
  console.warn(`[paletteLoader] Failed to load palette "${paletteName}"`, error)
}
...
function getColorsFromImage(image) {
  ...
  for (let x = 0; x < width; x++) {
    ...
    colors.push(rgbToHex(pixels[offset], pixels[offset + 1], pixels[offset + 2]))
  }
...
async function loadPaletteColors(paletteName) {
  if (!paletteName || typeof window === 'undefined' || typeof document === 'undefined') {
    return []
  }

  const src = `/assets/images/menu/${paletteName}.png`
