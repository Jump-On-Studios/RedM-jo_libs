...
export function requirementError(requirement: PriceRequirement): string | null {
  if (requirement.type === 'item') {
    if (!requirement.itemName?.trim()) {
      return useLangStore().getString('itemNameRequired', 'Item name is required.')
    }
  ...
  if (requirement.type === 'money') {
    if (value < 0) {
      return useLangStore().getString('moneyNegative', 'Money cannot be negative.')
    }
    return null
  }
...
export function summarizeOption(option: PriceOption): string {
  if (option.requirements.length === 0) {
    return useLangStore().getString('noRequirementSet', 'No requirement set')
  }
  if (isFreeOption(option)) return useLangStore().getString('free', 'Free')
