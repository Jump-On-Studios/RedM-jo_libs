...
// Ported from source-repositories/Minigame/src/directives/ui-scaler.ts with two
// deliberate differences:
//  - the reference height is 1080, the viewport every px token of this panel is
//    calibrated against (Minigame uses 1024 for its own layouts);
//  - the scale is derived from innerHeight instead of outerHeight. Both are the
//    render height in the game, where CEF draws the NUI without window chrome,
//    but only innerHeight is correct in a dev browser, where outerHeight also
//    counts the title bar and the tabs.
